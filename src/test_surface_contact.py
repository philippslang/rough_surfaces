...
class TestSurfaceContact(unittest.TestCase):
    ...
    def test_hertz(self):
        ...
        for force in self.F:
            ...
            # check
            if verbose:
                print(_rel_err(contact_radius_num, contact_radius))
                print(_rel_err(approach_num, approach))
                print(_rel_err(p_max_num, p_max))
            assert(np.isclose(contact_radius, contact_radius_num, rtol=0.01))
            assert(np.isclose(approach, approach_num, rtol=0.01))
            assert(np.isclose(p_max, p_max_num, rtol=0.01))
